...
public class EmployeeNewSeriesActorContoller {
    ...
    private void SubmitButtonOnACtion(ActionEvent e) {
        if ( RightTextField2.getText().isBlank() || RightTextField1.getText().isBlank()){
            ...
            try {
                ...
                insertRecord = "insert into season_actor values(?,?)";
                ...
                try {
                    App.connectDB.createStatement().executeUpdate( "insert into log values(null,\""+App.email+"\",now(), \"INSERT\", 0, \"season_actor\")");
                } catch (Exception x) {
                    x.printStackTrace();
                }
            }
