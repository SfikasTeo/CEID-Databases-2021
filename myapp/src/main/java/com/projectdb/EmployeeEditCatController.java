...
public class EmployeeEditCatController {
    ...
    private void SubmitButtonOnACtion(ActionEvent e){
        ...
        try {
            /* Update Lang */
            String updateLang = "update category set category_id=?,catname=? where category_id=?";
            ...
            try {
                App.connectDB.createStatement().executeUpdate( "insert into log values(null,\""+App.email+"\",now(), \"UPDATE\", 0, \"category\")");
            } catch (Exception x) {
                x.printStackTrace();
            }
        }
    ...
    private void initializeLeftLabels(){
        String query = "select * from category where category_id=\"" + String.valueOf(EmployeeController.editID) + "\"";
        ...
        try {
            ...
            if (Data.next()){
                ...
                LeftLabel1.setText( String.valueOf( Data.getInt("category_id")) );
                LeftLabel2.setText( String.valueOf( Data.getString("catname")) );
