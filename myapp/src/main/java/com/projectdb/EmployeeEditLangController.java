...
public class EmployeeEditLangController {
    ...
    private void SubmitButtonOnACtion(ActionEvent e){
        ...
        try {
            /* Update Lang */
            String updateLang = "update lang set lang_id=?,lang_name=? where lang_id=?";
            ...
            try {
                App.connectDB.createStatement().executeUpdate( "insert into log values(null,\""+App.email+"\",now(), \"UPDATE\", 0, \"lang\")");
            } catch (Exception x) {
                x.printStackTrace();
            }
        }
    ...
    private void initializeLeftLabels(){
        String query = "select * from lang where lang_id=\"" + String.valueOf(EmployeeController.editID) + "\"";
        ...
        try {
            ...
            if (Data.next()){
                ...
                LeftLabel1.setText( String.valueOf( Data.getInt("lang_id")) );
                LeftLabel2.setText( String.valueOf( Data.getString("lang_name")) );
