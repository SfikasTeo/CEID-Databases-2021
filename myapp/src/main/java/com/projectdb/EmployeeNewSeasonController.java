...
public class EmployeeNewSeasonController {
    /* Modules */
    @FXML 
    ...
    private void SubmitButtonOnACtion(ActionEvent e) {
        if ( RightTextField2.getText().isBlank() || RightTextField3.getText().isBlank() || RightTextField4.getText().isBlank() || RightTextField5.getText().isBlank() ){
            ...
            try {
                ...
                if ( RightTextField1.getText().isBlank() ){
                    insertRecord = "insert into season values(null,?,?,?,?)";
                    ...
                    insertRecord = "insert into season values(?,?,?,?,?,?)";
                    pstmt = App.connectDB.prepareStatement(insertRecord);
                ...
                try {
                    App.connectDB.createStatement().executeUpdate( "insert into log values(null,\""+App.email+"\",now(), \"INSERT\", 0, \"season\")");
                } catch (Exception x) {
                    x.printStackTrace();
                }
            }
