...
public class EmployeeNewFilmActorController {
    ...
    private void SubmitButtonOnACtion(ActionEvent e) {
        if ( RightTextField2.getText().isBlank() || RightTextField1.getText().isBlank()){
            ...
            try {
                ...
                insertRecord = "insert into film_actor values(?,?)";
                ...
                try {
                    App.connectDB.createStatement().executeUpdate( "insert into log values(null,\""+App.email+"\",now(), \"INSERT\", 0, \"film_actor\")");
                } catch (Exception x) {
                    x.printStackTrace();
                }
            }
