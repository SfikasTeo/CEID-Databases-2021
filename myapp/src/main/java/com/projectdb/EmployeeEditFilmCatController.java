...
public class EmployeeEditFilmCatController {
    ...
    private void SubmitButtonOnACtion(ActionEvent e){
        ...
        try {
            ...
            String updateLang = "update film_category set film_id=?, category_id=? where film_id=? and category_id=?";
            ...
            try {
                App.connectDB.createStatement().executeUpdate( "insert into log values(null,\""+App.email+"\",now(), \"UPDATE\", 0, \"film_category\")");
            } catch (Exception x) {
                x.printStackTrace();
            }
        }
