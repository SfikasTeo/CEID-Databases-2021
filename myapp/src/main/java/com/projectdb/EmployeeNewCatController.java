...
public class EmployeeNewCatController {
    ...
    private void SubmitButtonOnACtion(ActionEvent e) {
        if ( RightTextField2.getText().isBlank() ){
            ...
            try {
                ...
                try {
                    App.connectDB.createStatement().executeUpdate( "insert into log values(null,\""+App.email+"\",now(), \"INSERT\", 0, \"category\")");
                } catch (Exception x) {
                    x.printStackTrace();
                }
            }
