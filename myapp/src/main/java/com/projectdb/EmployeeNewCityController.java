...
public class EmployeeNewCityController {
        ...
        if ( RightTextField2.getText().isBlank() || RightTextField3.getText().isBlank()  ){
            ...
            try {
                ...
                if ( RightTextField1.getText().isBlank() ){
                    insertRecord = "insert into city values(null,?,?)";
                    ...
                    insertRecord = "insert into city values(?,?,?)";
                    pstmt = App.connectDB.prepareStatement(insertRecord);
                ...
                try {
                    App.connectDB.createStatement().executeUpdate( "insert into log values(null,\""+App.email+"\",now(), \"INSERT\", 0, \"city\")");
                } catch (Exception x) {
                    x.printStackTrace();
                }
            }
