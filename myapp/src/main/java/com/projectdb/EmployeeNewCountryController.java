...
public class EmployeeNewCountryController {
    ...
    private void SubmitButtonOnACtion(ActionEvent e) {
        if ( RightTextField2.getText().isBlank() ){
            ...
            try {
                ...
                try {
                    App.connectDB.createStatement().executeUpdate( "insert into log values(null,\""+App.email+"\",now(), \"INSERT\", 0, \"country\")");
                } catch (Exception x) {
                    x.printStackTrace();
                }
            }
