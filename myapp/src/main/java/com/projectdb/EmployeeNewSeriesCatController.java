...
public class EmployeeNewSeriesCatController {
    ...
    private void SubmitButtonOnACtion(ActionEvent e) {
        if ( RightTextField2.getText().isBlank() || RightTextField1.getText().isBlank()){
            ...
            try {
                ...
                insertRecord = "insert into series_category values(?,?)";
                ...
                try {
                    App.connectDB.createStatement().executeUpdate( "insert into log values(null,\""+App.email+"\",now(), \"INSERT\", 0, \"series_category\")");
                } catch (Exception x) {
                    x.printStackTrace();
                }
            }
