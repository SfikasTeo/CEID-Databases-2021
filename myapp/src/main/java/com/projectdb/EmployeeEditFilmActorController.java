...
public class EmployeeEditFilmActorController {
    ...
    private void SubmitButtonOnACtion(ActionEvent e){
        ...
        try {
            /* Update Lang */
            String updateLang = "update film_actor set actor_id=?, film_id=? where actor_id=? and film_id=?";
            ...
            try {
                App.connectDB.createStatement().executeUpdate( "insert into log values(null,\""+App.email+"\",now(), \"UPDATE\", 0, \"film_actor\")");
            } catch (Exception x) {
                x.printStackTrace();
            }
        }
