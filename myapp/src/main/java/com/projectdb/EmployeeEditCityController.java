...
public class EmployeeEditCityController {
    ...
    private void SubmitButtonOnACtion(ActionEvent e){
        ...
        try {
            /* Update Actor */
            String updateLang = "update city set city_id=?, city=?, country_id=? where city_id=?";
            ...
            try {
                App.connectDB.createStatement().executeUpdate( "insert into log values(null,\""+App.email+"\",now(), \"UPDATE\", 0, \"city\")");
            } catch (Exception x) {
                x.printStackTrace();
            }
        }
    ...
    private void initializeLeftLabels(){
        String query = "select * from city where city_id=\"" + String.valueOf(EmployeeController.editID) + "\"";
        ...
        try {
            ...
            if (Data.next()){
                ...
                LeftLabel1.setText( String.valueOf( Data.getInt("city_id")) );
                LeftLabel2.setText( String.valueOf( Data.getString("city")) );
                LeftLabel3.setText( String.valueOf( Data.getString("country_id")) );
            }
            //Data.close();
